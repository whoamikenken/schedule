...
<div class="card shadow animate__animated animate__fadeInRight">
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a href="javascript:void(0);" class="btn btn-primary mb-2 addbtn"><i class="bi bi-plus-circle"></i> Add Location</a>
            </div>
            <div class="col-sm-7">
...
<script>
    ...
    function LocationList(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('location/table')}}",
            data: {},
            async: false,
            success:function(response){
                $("#LocationTable").html(response);
                tableObj = $("#LocationTable").DataTable({
                    dom: 'Bfrtip',
                    buttons: [
                    {
                        extend: 'pdfHtml5',
                        ...
                        customize: function ( doc ) {
                            ...
                            $("#LocationTable").find('tbody tr:first-child td').each(function(){
    ...
    $(".addbtn").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('location/getModal')}}",
            data: {
                uid: uid
            },
            success: function(response) {
                $("#modal-view").modal('toggle');
                $("#modal-view").find(".modal-title").text("Add Location");
    ...
    $("#LocationTable").on("click", ".editbtn", function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                $("#modal-view").modal('toggle');
                $("#modal-view").find(".modal-title").text("Edit Location");
                $("#modal-view").find("#modal-display").html(response);
            }
        });
    });

    $("#LocationTable").on("click", ".delbtn", function() {
